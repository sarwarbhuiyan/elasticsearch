...
h3. Building using Google Bazel

This is experimental and is only for demonstration purposes. So far only the elasticsearch jar can be built. To do so, download the source to "Bazel":http://github.com/google/bazel and follow the Bazel build instructions.

Then change directory to the elasticsearch folder and run bazel build --javacopt "-source 1.7 -target 1.7" core/elasticsearch-core

The jar should be viewable in bazel-bin/core

TODO: produce the release as the maven build does
TODO: Build and run tests using Bazel
